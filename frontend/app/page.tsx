                ...
                <div className="cluster-blocks">
                  {people.map((person) => (
                    <button aria-pressed={selectedId === person.id} className={`community-block ${activePersonIds.has(person.id) ? "community-block-active" : ""}`} key={person.id} onClick={() => onSelect(person.id)} type="button">
                      <span className="block-icon block-icon-person"><BlockIcon kind="person" /></span><span className="block-copy"><strong>{person.name}</strong><small>{person.capabilities.length > 0 ? humanize(person.capabilities[0]) : "Learner"}</small></span>{activePersonIds.has(person.id) ? <Check aria-label="Assigned" className="block-check" size={15} weight="bold" /> : null}
                    </button>
                  ))}
                  {spaces.map((space) => (
                    <button aria-pressed={selectedId === space.id} className={`community-block ${selectedId === space.id ? "community-block-selected" : ""}`} key={space.id} onClick={() => onSelect(space.id)} type="button">
                      <span className="block-icon block-icon-space"><BlockIcon kind="space" /></span><span className="block-copy"><strong>{space.name}</strong><small>Capacity {space.capacity} · {space.features.slice(0, 2).map(humanize).join(" · ")}</small></span>
                    </button>
                  ))}
                  {resources.map((resource) => (
                    <button aria-pressed={selectedId === resource.id} className={`community-block ${selectedId === resource.id ? "community-block-selected" : ""}`} key={resource.id} onClick={() => onSelect(resource.id)} type="button">
                      <span className="block-icon block-icon-resource"><BlockIcon kind="resource" /></span><span className="block-copy"><strong>{resource.name}</strong><small>{resource.quantity} available · {resource.shareable ? "Shareable" : "Reserved"}</small></span>
                    </button>
                  ))}
                </div>
              </article>
    ...
    <section className="rail-region" aria-labelledby="initiatives-title">
      ...
      <div className="initiative-list">
        {initiatives.map((initiative, index) => {
          ...
          const shownStatus = isPathTarget && !result ? plan.target_status_after : result?.status;
          return (
            <button aria-current={selectedId === initiative.id ? "true" : undefined} className={`initiative-card ${selectedId === initiative.id ? "initiative-card-selected" : ""}`} key={initiative.id} onClick={() => onSelect(initiative.id)} type="button">
              <div className="initiative-card-topline"><span className="initiative-index mono">{String(index + 1).padStart(2, "0")}</span><StatusBadge status={shownStatus} label={isPathTarget && shownStatus !== "OPTIMAL" && shownStatus !== "FEASIBLE" ? "Path found" : undefined} /></div>
              ...
              <div className="initiative-card-footer"><span className="mono">{compactId(initiative.id)}</span><CaretRight aria-hidden="true" size={17} weight="bold" /></div>
            </button>
          );
        })}
      </div>
...
function ActionButton({ label, hint, icon, onClick, disabled, loading, complete, primary }: { label: string; hint: string; icon: React.ReactNode; onClick: () => void; disabled?: boolean; loading?: boolean; complete?: boolean; primary?: boolean }) {
  return (
    <Button className={`action-button ${primary ? "action-button-primary" : ""} ${complete ? "action-button-complete" : ""}`} disabled={disabled || loading} onClick={onClick} size="2" variant={primary ? "solid" : "outline"}>
      <span className="action-button-icon">{loading ? <CircleNotch aria-hidden="true" className="spin" size={18} weight="bold" /> : complete ? <Check aria-hidden="true" size={18} weight="bold" /> : icon}</span><span className="action-button-copy"><strong>{label}</strong><small>{loading ? "Working…" : hint}</small></span>{!loading && !complete ? <ArrowRight aria-hidden="true" className="action-arrow" size={16} weight="bold" /> : null}
    </Button>
  );
}
...
function AnalysisPanel({ result, community, requestState, error, onRetry }: { result: InitiativeAnalysisResult | null; community: CommunityState; requestState: RequestState; error?: UiError; onRetry: () => void }) {
  ...
  return <div className="analysis-success"><div className="result-callout result-callout-success"><div className="result-callout-icon"><CheckCircle aria-hidden="true" size={22} weight="fill" /></div><div><strong>Assembly found for this brief.</strong><span>Every shown assignment and fact below came from the solver response.</span></div><div className="result-objective"><span>Objective</span><strong className="mono">{result.objective_value ?? "—"}</strong></div></div>{result.assembly_trace.length > 0 ? <TraceTable entries={result.assembly_trace} community={community} /> : <div className="empty-subtle">The solver returned no assembly trace entries for this result.</div>}</div>;
}
...
function BlockerPanel({ explanation, requestState, error, onRetry }: { explanation: ExplainResponse | null; requestState: RequestState; error?: UiError; onRetry: () => void }) {
  ...
  return <div className="blocker-list">{explanation.blocking_requirement_sets.map((set, index) => <div className="blocker-item" key={`${set.groups.join("-")}-${index}`}><div className="blocker-item-heading"><span className="blocker-number mono">0{index + 1}</span><div><strong>{set.groups.map(humanize).join(" + ")}</strong><span>{set.restored_feasibility_when_relaxed ? "Feasibility returns when this set is relaxed." : "Relaxing this set did not restore feasibility."}</span></div></div><div className="fact-list">{set.facts.map((fact, factIndex) => <div className="fact-row" key={`${fact.capability ?? fact.language ?? fact.requirement_id ?? "fact"}-${factIndex}`}><span className="fact-label">{fact.capability ? humanize(fact.capability) : fact.language ? `Language: ${fact.language}` : fact.requirement_id ?? "Requirement"}</span>{fact.required !== null && fact.available !== null ? <span className="fact-value"><strong className="mono">{fact.available}</strong><span>available of</span><strong className="mono">{fact.required}</strong><span>required</span></span> : null}{fact.note ? <span className="fact-note">{fact.note}</span> : null}{fact.relevant_ids.length > 0 ? <span className="fact-ids mono">Relevant: {fact.relevant_ids.join(", ")}</span> : null}</div>)}</div></div>)}<div className="method-line"><MagnifyingGlass aria-hidden="true" size={15} weight="bold" />{explanation.method.replaceAll("_", " ")} · {explanation.solver_runs} bounded solver runs</div></div>;
}
...
function TransitionPanel({ transition, successorResult, requestState, verifyState, error, verifyError, onRetry, onVerify }: { transition: TransitionResponse | null; successorResult: InitiativeAnalysisResult | null; requestState: RequestState; verifyState: RequestState; error?: UiError; verifyError?: UiError; onRetry: () => void; onVerify: () => void }) {
  ...
  if (error) return <ErrorNotice error={error} onRetry={onRetry} retryLabel="Apply again" />;
...
function InspectorRow({ label, value, mono = false }: { label: string; value?: number | string; mono?: boolean }) { return <div className="inspector-row"><span>{label}</span><strong className={mono ? "mono" : ""}>{value ?? "—"}</strong></div>; }

function ActionWorkspace({ selectedInitiative, selectedResult, community, demo, requestStates, requestErrors, explanation, unlock, plan, transition, verifiedResult, handlers }: { selectedInitiative: InitiativeBlueprint | null; selectedResult: InitiativeAnalysisResult | null; community: CommunityState; demo: DemoFixture; requestStates: Record<RequestKey, RequestState>; requestErrors: Partial<Record<RequestKey, UiError>>; explanation: ExplainResponse | null; unlock: UnlockResponse | null; plan: PlanResponse | null; transition: TransitionResponse | null; verifiedResult: InitiativeAnalysisResult | null; handlers: { compile: () => void; assemble: () => void; explain: () => void; unlock: () => void; apply: () => void; verify: () => void } }) {
  const selectedStatus = verifiedResult?.status ?? selectedResult?.status;
  const isBlocked = selectedStatus === "INFEASIBLE";
  const canUnlock = isBlocked && requestStates.unlock !== "loading";
  ...
  return <section className="workspace-region" aria-labelledby="workspace-title"><div className="workspace-heading"><div><p className="section-kicker">Action workspace</p><h2 id="workspace-title">Turn a brief into a verified next state</h2></div>{selectedInitiative ? <StatusBadge status={selectedStatus} /> : null}</div>{selectedInitiative ? <div className="selected-brief"><div className="selected-brief-index mono">{selectedInitiative.id.slice(0, 2)}</div><div><strong>{selectedInitiative.name}</strong><span>{selectedInitiative.roles.map((role) => role.label).join(" · ")}</span></div><span className="mono selected-brief-id">{selectedInitiative.id}</span></div> : null}<div className="action-grid" aria-label="Six action journey"><ActionButton label="COMPILE COMMUNITY" hint="Load model evidence" icon={<Lightning aria-hidden="true" size={18} weight="fill" />} onClick={handlers.compile} loading={requestStates.analyse === "loading"} complete={requestStates.analyse === "success"} primary disabled={!demo} /><ActionButton label="ASSEMBLE NOW" hint="Solve selected brief" icon={<ListChecks aria-hidden="true" size={18} weight="duotone" />} onClick={handlers.assemble} loading={requestStates.analyse === "loading"} complete={Boolean(selectedResult) && requestStates.analyse === "success"} disabled={!selectedInitiative || !demo} /><ActionButton label="WHY BLOCKED?" hint="Inspect bounded facts" icon={<MagnifyingGlass aria-hidden="true" size={18} weight="duotone" />} onClick={handlers.explain} loading={requestStates.explain === "loading"} complete={Boolean(explanation)} disabled={!canExplain} /><ActionButton label="FIND MINIMUM UNLOCK" hint="Compare interventions" icon={<LockKeyOpen aria-hidden="true" size={18} weight="duotone" />} onClick={handlers.unlock} loading={requestStates.unlock === "loading" || requestStates.plan === "loading"} complete={Boolean(unlock && plan)} disabled={!canUnlock} /><ActionButton label="APPLY CATALYST" hint="Create successor state" icon={<Sparkle aria-hidden="true" size={18} weight="duotone" />} onClick={handlers.apply} loading={requestStates.transition === "loading"} complete={successorExists} disabled={!catalystReady || successorExists} /><ActionButton label="VERIFY NEW STATE" hint="Re-solve with evidence" icon={<CheckCircle aria-hidden="true" size={18} weight="duotone" />} onClick={handlers.verify} loading={requestStates.verify === "loading"} complete={Boolean(verifiedResult)} disabled={!successorExists || Boolean(verifiedResult)} /></div><div className="workspace-divider"><span>Evidence returned by the selected action</span><span className="divider-line" /></div><AnalysisPanel result={selectedResult} community={community} requestState={requestStates.analyse} error={requestErrors.analyse} onRetry={handlers.assemble} />{isBlocked ? <div className="evidence-section"><div className="evidence-heading"><span className="evidence-step mono">02</span><div><strong>Why this is blocked</strong><span>Requirement facts, not a generic warning.</span></div></div><BlockerPanel explanation={explanation} requestState={requestStates.explain} error={requestErrors.explain} onRetry={handlers.explain} /></div> : null}{unlock || requestStates.unlock === "loading" || requestStates.unlock === "error" ? <div className="evidence-section"><div className="evidence-heading"><span className="evidence-step mono">03</span><div><strong>Minimum unlock and path</strong><span>Finite intervention catalogue with a bounded successor trace.</span></div></div><UnlockPanel unlock={unlock} plan={plan} actions={demo.actions} requestState={requestStates.unlock} planState={requestStates.plan} error={requestErrors.unlock} planError={requestErrors.plan} onRetry={handlers.unlock} /></div> : null}{transition || requestStates.transition === "loading" || requestStates.transition === "error" ? <div className="evidence-section"><div className="evidence-heading"><span className="evidence-step mono">04</span><div><strong>Capability transition</strong><span>Immutable diff followed by independent verification.</span></div></div><TransitionPanel transition={transition} successorResult={verifiedResult} requestState={requestStates.transition} verifyState={requestStates.verify} error={requestErrors.transition} verifyError={requestErrors.verify} onRetry={handlers.apply} onVerify={handlers.verify} /></div> : null}{requestErrors.analyse && !selectedResult ? <ErrorNotice error={requestErrors.analyse} onRetry={handlers.compile} retryLabel="Compile again" /> : null}</section>;
}
...
export default function Home() {
  ...
  const loadDemo = useCallback(async () => {
    ...
    try {
      ...
      setDemo(nextDemo); setCommunity(nextDemo.community); setSelectedId(nextDemo.initiatives[0]?.id ?? ""); setSelectedBlockId(""); setAnalyses({}); setCompile(null); setExplanation(null); setUnlock(null); setPlan(null); setTransition(null); setVerifiedResult(null); setRequestStates({ ...initialRequestStates, demo: "success" }); setRequestErrors({});
  ...
  const compileCommunity = useCallback(async (initiativeIds?: string[]) => {
    ...
    if (!response) return;
    setCompile(response.compile); setAnalyses((current) => ({ ...current, ...Object.fromEntries(response.results.map((result) => [result.initiative_id, result])) }));
  }, [community, demo, runRequest]);
  const explainSelected = useCallback(async () => { if (!community || !selectedId) return; const response = await runRequest("explain", () => api.explain(community, selectedId)); if (response) setExplanation(response); }, [community, runRequest, selectedId]);
  const findUnlock = useCallback(async () => {
    ...
    setUnlock(response); setPlan(null); const planResponse = await runRequest("plan", () => api.plan(community, selectedId, demo.actions)); if (planResponse) setPlan(planResponse);
  ...
  const applyCatalyst = useCallback(async () => {
    ...
    const response = await runRequest("transition", () => api.transition(community, actionId, demo.actions)); if (response) { setTransition(response); setVerifiedResult(null); setAnalyses({}); setCompile(null); setExplanation(null); setCommunity(response.successor_state); }
  ...
  const verifyNewState = useCallback(async () => {
    ...
    if (result) { setVerifiedResult(result); setAnalyses((current) => ({ ...current, [result.initiative_id]: result })); }
  }, [demo, plan, runRequest, selectedId, transition, unlock]);
  const selectInitiative = useCallback((id: string) => {
    if (!demo?.initiatives.some((initiative) => initiative.id === id)) return;
    setSelectedId(id); setSelectedBlockId(""); setExplanation(null); setUnlock(null); setPlan(null); setTransition(null); setVerifiedResult(null); setRequestStates((current) => ({ ...current, explain: "idle", unlock: "idle", plan: "idle", transition: "idle", verify: "idle" }));
  ...
  if (isDemoLoading && !demo) return <Theme appearance="light" accentColor="indigo" grayColor="slate" panelBackground="solid" radius="medium" scaling="100%"><main className="app-shell loading-shell"><header className="product-header"><div className="brand-lockup"><span className="brand-mark"><Sparkle aria-hidden="true" size={18} weight="fill" /></span><span>ASSEMBLE</span></div><span className="header-context">Community Systems Compiler</span></header><div className="page-loading"><div className="loading-orbit"><CircleNotch aria-hidden="true" className="spin" size={23} weight="bold" /></div><h1>Opening the community fixture</h1><p>Loading people, places and shared resources before the first compile.</p><div className="loading-layout"><div className="loading-canvas"><InlineSkeleton lines={5} /></div><div className="loading-rail"><InlineSkeleton lines={7} /></div></div></div></main></Theme>;
  if (!demo || !community) { const error = requestErrors.demo ?? { code: "SERVICE_UNAVAILABLE", message: "The community fixture could not be loaded." }; return <Theme appearance="light" accentColor="indigo" grayColor="slate" panelBackground="solid" radius="medium" scaling="100%"><main className="app-shell error-shell"><header className="product-header"><div className="brand-lockup"><span className="brand-mark"><Sparkle aria-hidden="true" size={18} weight="fill" /></span><span>ASSEMBLE</span></div><span className="header-context">Community Systems Compiler</span></header><div className="fatal-state"><div className="empty-icon empty-icon-error"><WarningCircle aria-hidden="true" size={25} weight="fill" /></div><h1>We could not open the planning table.</h1><p>Check that the planning service is running, then try the deterministic fixture again.</p><ErrorNotice error={error} onRetry={() => void loadDemo()} retryLabel="Reload fixture" /></div></main></Theme>; }
  return <Theme appearance="light" accentColor="indigo" grayColor="slate" panelBackground="solid" radius="medium" scaling="100%"><main className="app-shell"><header className="product-header"><div className="brand-lockup"><span className="brand-mark"><Sparkle aria-hidden="true" size={18} weight="fill" /></span><span>ASSEMBLE</span></div><div className="header-context"><span>Community Systems Compiler</span><span className="header-divider" /><span className="mono">{demo.fixture_version}</span></div><Button className="reset-button" onClick={() => void loadDemo()} size="1" variant="ghost"><ArrowClockwise aria-hidden="true" size={16} weight="bold" /> Demo reset</Button></header>{requestErrors.demo ? <ErrorNotice error={requestErrors.demo} onRetry={() => void loadDemo()} retryLabel="Reload fixture" /> : null}<div className="page-intro"><div><p className="eyebrow">A living planning table</p><h1>Make the community&apos;s capacity visible.</h1><p className="intro-copy">Assemble people, places and resources into initiatives that can be explained, unlocked and verified.</p></div><div className="intro-status"><span className="intro-status-icon"><Lightning aria-hidden="true" size={18} weight="fill" /></span><div><strong>{compile ? "Community compiled" : "Ready to compile"}</strong><span>{compile ? `${compile.people} people · ${compile.hard_constraints} constraints` : "Start with the full fixture to reveal returned solver evidence."}</span></div></div></div><div className="workspace-grid"><CommunityCanvas community={community} analyses={analyses} selectedId={selectedBlockId} onSelect={setSelectedBlockId} transition={transition} /><aside className="rail-column"><InitiativeRail initiatives={demo.initiatives} analyses={analyses} verifiedResult={verifiedResult} plan={plan} selectedId={selectedId} onSelect={selectInitiative} /><ActionWorkspace selectedInitiative={selectedInitiative} selectedResult={selectedResult} community={community} demo={demo} requestStates={requestStates} requestErrors={requestErrors} explanation={explanation} unlock={unlock} plan={plan} transition={transition} verifiedResult={verifiedResult} handlers={{ compile: () => void compileCommunity(), assemble: () => void compileCommunity([selectedId]), explain: () => void explainSelected(), unlock: () => void findUnlock(), apply: () => void applyCatalyst(), verify: () => void verifyNewState() }} /></aside></div><TechnicalInspector compile={compile} selectedResult={verifiedResult ?? selectedResult} explanation={explanation} unlock={unlock} plan={plan} transition={transition} fixtureVersion={demo.fixture_version} /><footer className="page-footer"><span><Info aria-hidden="true" size={15} weight="bold" /> Results remain bounded to this deterministic fixture and the returned solver state.</span><span className="mono">{community.state_id}</span></footer></main></Theme>;
}
