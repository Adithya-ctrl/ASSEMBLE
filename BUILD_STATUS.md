...
| Local identity, community membership and invitations | Builder green; independent UI acceptance pending | Dedicated signup/login, account, Security, Appearance, Collaboration, member, invitation and audit surfaces pass the Builder static and production-browser gate against the integrated SQLite backend; independent replay is still required |
...
- Interface: `15/15` focused frontend tests pass; TypeScript check passes; ESLint passes with zero warnings; Next.js 16.3.3 production build passes.
- Modular routes: Overview, demo Community, Initiatives, dynamic Initiative Proof, Projects, Project Proof, identity entry, Settings, Collaboration spaces and dynamic community administration resolve as real routes. `/preferences` redirects to Settings. Unknown and malformed Initiative Proof paths fail visibly without selecting a fallback initiative.
...
- Full platform parity: accepted M6 evidence covers the five planning destinations and proof workflow. The Phase A Builder replay adds discoverable guest account/Settings access plus authenticated Collaboration and Administrator controls at 320, 390 and 1440, with no document overflow or effective sub-44px control; independent Phase A replay is pending.
...
The current Builder checkpoint connects local authentication, invitations, community roles, membership and SQLite persistence to dedicated frontend surfaces without changing the accepted planning proof chain. The account menu supports guest entry and authenticated profile/logout; Settings separates Account, Security and Appearance; Collaboration separates persisted spaces from the fictional demo; and only Administrators receive member, invitation and audit controls. Browser replay covered stable 401/403/409 handling, scheduled session expiry, immediate cached-session invalidation after revocation, authoritative role-loss refresh, 204 logout, profile update, one-time token removal, revoke, Viewer read-only behavior, scoped live announcements, failure visibility, and SQLite restart recovery. This is Builder evidence pending independent acceptance. Backend-only M7 stress, recompilation and capability-frontier analyses still have no frontend route. Reasoning, Project and M7 endpoints remain outside community-role permissions. Projects and Project proof remain in memory.
