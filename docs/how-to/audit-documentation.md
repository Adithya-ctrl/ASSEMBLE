...
Replay the identity/Collaboration UI and API against one private SQLite file across an application restart, and replay all three M7 routes against authoritative S0. Confirm the precise boundary: auth/community/invitation state persists and has a frontend workflow; M7 remains backend-only; solver, reasoning, Project and M7 routes are not role-gated; persisted Collaboration spaces do not alter the fictional fixture; Projects and proof state remain in memory; and counterfactual receipts cannot become operational Project lineage. Architectural history belongs only in ADRs.
