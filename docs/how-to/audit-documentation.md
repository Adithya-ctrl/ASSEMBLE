...
The adversarial read-only audit adds route/source inventory, requirement/traceability coverage, current-page inventory, adversarial-artifact presence, and exact stale-claim checks. Its machine-readable result must be `PASS` with zero findings. A structural or source-only pass remains supporting evidence; it does not turn unexecuted browser rows into acceptance.
...
- [`../ADVERSARIAL_ACCEPTANCE_REPORT.md`](../ADVERSARIAL_ACCEPTANCE_REPORT.md), treating it as supporting evidence while `BUILD_STATUS.md` remains the canonical status and count owner.
...
For an adversarial milestone, compare completed and partial A–Z rows with the report's explicit gap list. Cross-browser execution, requested 400% zoom, the full state × viewport × theme screenshot product, a full screen-reader journey, every mounted async/parser/RBAC/race permutation, every named boss fight, mutation testing, and randomized browser monkey testing remain unverified until executed. Never promote a bounded Chrome pass to exhaustive release acceptance.
