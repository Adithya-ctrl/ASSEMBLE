...
The existing Next.js same-origin proxy is the current browser boundary. The frontend provides signup, login, session bootstrap, logout, profile and password Settings, collaboration-space creation and listing, recipient-bound invitation acceptance, and Administrator member, role, invitation and audit workflows. Browser requests remain same-origin and include the host-only HttpOnly session cookie without exposing its value to JavaScript. If a later frontend calls FastAPI cross-origin and sends cookies directly, review CORS origin, credentials and headers separately.
...
Run the focused auth suite and complete backend suite with a private temporary `ASSEMBLE_AUTH_DB_PATH`, then restart the application against one dedicated persistent file and replay signup → community → invitation → accept → role check. For the browser gate, also verify session bootstrap, account navigation, profile/password changes, 204 logout, one-time token removal, authoritative 401/403 invalidation, keyboard announcements and mobile/desktop parity. Record only the latest integrated counts; isolated branch counts are not a substitute for the control centre's replay. Local browser acceptance is not public-deployment acceptance.
