...
Run the focused auth suite and complete backend suite with a private temporary `ASSEMBLE_AUTH_DB_PATH`, then restart the application against one dedicated persistent file and replay signup → community → invitation → accept → role check. Record only the latest integrated counts; isolated branch counts are not a substitute for the control centre's replay. These are backend integration gates, not frontend availability or public-deployment acceptance.
