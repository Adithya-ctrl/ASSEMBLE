...
## Next-milestone governance — not current capability

A future separately contracted milestone must replace the dense all-in-one planning surface with modular, single-purpose product areas and progressive disclosure. Community inventory, initiatives, intervention planning, and Projects should have predictable detail views rather than being dumped into one dashboard. Community Administrator, Coordinator, Member, and Viewer experiences require explicit role, permission, data, and navigation contracts before implementation. Mobile and desktop must expose the same capabilities. Immich and Outline are interaction-level references for calm shells, restrained density, onboarding, empty states, and predictable navigation only; their branding and compositions must not be copied. None of these future roles, navigation changes, authentication controls, or collaboration capabilities is implemented by the current build.
...
- **FR-002 — Present equivalent inventory views.** The interface shall offer labelled Graph View and List View controls that expose equivalent IDs, organisations, capabilities, languages, availability, capacity, quantity, and features from the same state.
- **FR-003 — Select an initiative.** The user shall be able to select one declared initiative and see its roles, time, resource requirements, and current proof status.
...
- **FR-014 — Inspect source proof.** The Project shall expose its fresh verification, `source_plan_id`, exact catalyst path, base and verified state IDs, and catalyst outputs in a dedicated Technical Inspector section reached by a focusable source-proof control.
...
- **FR-016 — Control appearance and access representation.** The interface shall provide a semantic graph/list choice and a high-contrast toggle, preserve icon-plus-text statuses, and expose all actions through native labelled controls.
...
- **NFR-004 — Accessibility.** The current interface shall use semantic landmarks, heading order, labels, native keyboard controls, visible opaque focus, icon-plus-text status, scoped live announcements, at least 44-by-44 CSS-pixel targets, reduced-motion handling, and WCAG AA-oriented contrast practices.
- **NFR-005 — Responsive reflow.** Desktop, tablet, mobile, and 200% zoom layouts shall avoid horizontal document overflow; graph/list content shall remain equivalent when SVG seams are unavailable.
...
- **Given** the S0 fixture is loaded, **when** I switch between Graph View and List View, **then** both expose the same eight community blocks and core facts without losing keyboard access.
...
- **Given** a Project exists, **when** I activate View source proof, **then** the Technical Inspector opens, receives focus, and presents the Project response's fresh verification, source-plan ID, exact path, state lineage, and catalyst outputs.
...
- **Given** any supported presentation mode, **when** I complete the journey, **then** every control and status remains labelled, focus-visible, operable, announced, and free of horizontal document overflow.
...
- **Given** the same journey state at 320 and 1440 CSS pixels, **when** I inspect and operate the interface, **then** both widths expose the same controls, flows, three editable Project fields, inventory facts, proof evidence, and Project capabilities; only layout and label presentation may differ.
