...
ASSEMBLE is a localhost civic capacity-and-intervention planner for a deterministic fictional fixture. It inventories declared capacity, solves initiative feasibility, explains blockers, compares disclosed catalyst actions, verifies immutable successors, creates an executable Project from a fresh proof, and presents structural resilience analyses through a dedicated Resilience Lab. Its FastAPI boundary also provides local identity, persisted community membership and recipient-bound invitations.
...
- Email verification, MFA, OAuth, account recovery, or cloud identity.
...
The account and Collaboration surfaces expose FastAPI's local accounts plus Administrator, Coordinator, Member and Viewer memberships. Administrator-only controls are withheld from other roles, and every protected request is still authorised by the backend. These persisted roles govern only collaboration-space administration; they do not govern solver, reasoning, Project, or M7 routes. The Resilience route presents structural stress, minimum-disruption recovery and capability-frontier analysis from canonical S0 plus an authoritative verified path. It is read-only counterfactual analysis and cannot mutate the planning workflow or create Project lineage.
...
- **FR-021 — Stress a proved plan structurally.** The stress API shall reconstruct an authoritative source from exact S0 plus a unique 0–2 action path, require a feasible baseline, generate the complete canonical witness-derived one-fact perturbation catalogue, enforce its 20-entry ceiling before scenario solving, and report `RESILIENT`, `DEGRADED`, `CRITICAL` or `UNKNOWN` outcomes with resilience computed only over decisive results. The dedicated Resilience route shall expose this as a human-first Stress task and disable analysis while a transition awaits verification.
...
- **FR-023 — Compare the one-action capability frontier.** The frontier API shall apply each applicable authoritative action independently from the same reconstructed source, solve every initiative before and after it, track gains, losses and unknowns, and rank or Pareto-compare only candidates with complete decisive coverage. The Resilience route shall expose Stress, Recovery and Capability frontier as three progressively disclosed tasks. Every request shall send untouched canonical S0 plus `[]` or the fully applied and verified authoritative catalyst path; separate abort/generation lanes shall reject stale responses, a new Stress run shall invalidate Recovery, and no counterfactual receipt shall alter community, transition, Project or proof state. It shall never present a one-action result as a sequenced operational successor.

## Non-functional requirements

- **NFR-001 — Proof integrity and explainability.** Every analyser call shall preserve its submitted community and initiative, bind its returned status to the requested initiative, and enforce consistent status, witness and objective fields. Every feasible witness shall pass canonical replay of exact assignments, trace structure and domain facts, candidate start, sharing, objective, and all non-relaxed predicates before it is returned. Every blocker, unlock, transition, verification, readiness, and Project claim shall remain adjacent to machine-readable evidence or a direct source-proof path. UNKNOWN shall remain UNKNOWN; malformed analyser output shall fail as `ANALYSER_CONTRACT_ERROR`.
...
- **NFR-003 — Validation and security.** Backend models shall reject unknown fields, Stable IDs outside 1–256 characters, coercive integer/boolean/literal inputs, and collections above the exact ceilings documented in the security reference. Project creation shall fail closed on forged base content or lineage, unsafe client proof fields, invalid paths, blank normalized metadata, INFEASIBLE, UNKNOWN, or a malformed feasible witness and shall never emit a Project on failure.
- **NFR-004 — Accessibility.** The current interface shall use semantic landmarks, heading order, labels, native keyboard controls, visible opaque focus, icon-plus-text status, one scoped live announcement, at least 44-by-44 CSS-pixel targets, reduced-motion handling, and WCAG AA-oriented contrast practices across normal, dark, and high-contrast presentations. The calm workspace shall use one navigation system per viewport, one job and no more than three primary tasks per route, and progressive disclosure for technical proof.
...
- **NFR-006 — Performance budget.** Controls shall visibly react within 100 ms, use honest labelled loading for backend work, and avoid fake progress. Product imagery shall use responsive `next/image` delivery from the five compressed alpha WebP assets. Spatial motion shall remain an isolated CSS transform/opacity leaf with cleaned-up animation frames, static mobile and reduced-motion fallbacks, no per-frame React state, and no WebGL, 3D-framework, video, or heavy animation dependency. Solver execution remains bounded by its configured limit.
...
- **NFR-012 — Local identity security and durable lifecycle.** Passwords shall use canonical bounded `scrypt-v1` encoding with an exact 16-byte salt; session and invitation bearer secrets shall use 256-bit random values with digest-only storage; avatar URLs shall have a valid bounded port; auth bodies, fields and persisted rate buckets shall be bounded; unsafe requests shall enforce JSON, reject duplicate security-sensitive headers, and use an exact bounded canonical browser-origin allow-list that cannot be widened by Host or forwarded headers; auth namespace matching shall be segment-aware; and restart shall preserve valid lifecycle, role, revocation, expiry, rate and audit state. On POSIX, the auth directory and database runtime files shall be exactly `0700` and `0600`; unsafe existing modes fail closed, while bounded constructor-only initialization converges under concurrent cold start.
...
- **Given** an authoritative feasible source path, **when** I use Stress and submit one returned canonical perturbation to Recovery, **then** every one-fact scenario is accounted for, UNKNOWN remains explicit, any replacement witness reports only a proved minimum assignment-change count plus validated normal-burden evidence, and the analysis does not change the operational workflow.

### US-017 — Compare one catalyst's capacity effect

As a coalition planner, I want a one-action capability frontier so I can compare disclosed catalysts without confusing analytical receipts with operational successors. Maps to FR-023, NFR-002, NFR-004, NFR-013.
...
| US-016, US-017 | Resilience Lab stress, recovery and frontier evidence | Optional Resilience close; distinguish counterfactual analysis from operational Project lineage |
